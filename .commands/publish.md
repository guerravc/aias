...
PUBLISH COMPLETE:
  ...
  RHOAIAS.md: updated | deferred (consider /aias refresh-context) | skipped (consider /aias refresh-context) | not required
  Status: completed
```

The `RHOAIAS.md` line reads `rhoaias_update` from `status.md`: `done` → "updated", `skipped` → "skipped (consider /aias refresh-context)", `deferred` → "deferred (consider /aias refresh-context)", `null`/absent → "not required". This is informational only — no gate fires.
