...
- `aias-providers/<provider_id>/` — e.g., `aias-providers/atlassian/jira-field-mapping.md`, `aias-providers/atlassian/tracker-status-mapping.md`, `aias-providers/atlassian/confluence-config.md`

These files are generated by `/aias configure-providers` (AI-assisted MCP discovery) and referenced via `resource_files` in the provider config. Skills provide operational knowledge (MCP call sequences, safety rules); project-specific configuration lives in `aias-providers/`.
