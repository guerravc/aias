...
Canonical sources live in `aias/.canonical/`. Generated output goes to `aias-config/modes/`.
...
- `aias-config/modes/*.mdc` — Platform-specific mode rules
- `aias-config/rules/*.mdc` — Workspace rules (base + output-contract + continuous-improvement)
...
Each category has a configuration file in `aias-config/providers/`:
...
Categories with referenced documents (`tracker`, `knowledge`) also have a provider-specific subdirectory with project configuration files:

- `aias-config/providers/<provider_id>/` — e.g., `aias-config/providers/atlassian/jira-field-mapping.md`, `aias-config/providers/atlassian/tracker-status-mapping.md`, `aias-config/providers/atlassian/confluence-config.md`

These files are generated by `/aias configure-providers` (AI-assisted MCP discovery) and referenced via `resource_files` in the provider config. Skills provide operational knowledge (MCP call sequences, safety rules); project-specific configuration lives in `aias-config/providers/`.
