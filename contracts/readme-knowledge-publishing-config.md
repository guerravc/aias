...
The config is consumed via `config_source` in `aias-providers/knowledge-config.md`.
...
- `aias-providers/<provider_id>/confluence-config.md` (canonical, generated by `/aias configure-providers`)

Legacy location (deprecated in v7.5):

- `aias/.skills/<provider-skill>/confluence-config.md`

The active file must be referenced by `config_source` in `aias-providers/knowledge-config.md` and listed in `skill_binding.resource_files`.
